...
/* FISCALO — Reportes > Fiscales > 606 (compras DGII) */
...
export function Reporte606View({ nav }: { nav: Nav }) {
  ...
  const q = useApiQuery(
    ['reportes', '606', 'preview', periodo],
    () => getReporte606Preview(periodo),
    { keepPrevious: true },
  )
  const data = q.data
  ...
  const descargar = async () => {
    setDescargando(true)
    const tid = toast.loading('Generando 606…')
    try {
      const { blob, filename } = await downloadReporte606(periodo)
      downloadBlob(blob, filename)
      toast.success(`Descargado ${filename}`, { id: tid })
    } catch (e) {
      toast.error(e instanceof ApiError ? e.message : 'No se pudo descargar el 606.', { id: tid })
    } finally {
  ...
  return (
    <div className="page page-wide">
      <PageHead
        title="Reporte 606"
        sub="Compras de bienes y servicios — formato DGII"
        crumbs={[
      ...
      <div className="card card-pad mb-lg">
        <div className="row" style={{ gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div className="field" style={{ marginBottom: 0 }}>
            <label htmlFor="r606-anio">Año</label>
            <select id="r606-anio" className="select" value={anio} onChange={(e) => setAnio(e.target.value)} style={{ width: 'auto' }}>
              {aniosDisponibles().map((y) => <option key={y} value={String(y)}>{y}</option>)}
            </select>
          </div>
          <div className="field" style={{ marginBottom: 0 }}>
            <label htmlFor="r606-mes">Mes</label>
            <select id="r606-mes" className="select" value={mes} onChange={(e) => setMes(e.target.value)} style={{ width: 'auto' }}>
              {MESES.map((nombre, i) => {
      ...
      {data && data.cantidad > 0 && (
        <div className="kpi-grid mb-lg">
          <Total label="Total facturado" value={data.totales.total_facturado} icon="receipt" color="var(--success)" soft="var(--success-soft)" />
          <Total label="ITBIS facturado" value={data.totales.itbis_facturado} icon="landmark" color="var(--danger)" soft="var(--danger-soft)" />
          <Total label="Monto bienes" value={data.totales.monto_bienes} icon="package" color="var(--accent)" soft="var(--accent-soft)" />
          <Total label="Monto servicios" value={data.totales.monto_servicios} icon="briefcase" color="#7a5ae0" soft="rgba(122,90,224,.12)" />
        </div>
      )}
      ...
      <Card noPad>
        {q.loading ? (
          ...
          <ErrorState title="No se pudo cargar el reporte 606" onRetry={q.reload}>{q.error}</ErrorState>
        ) : vacio ? (
          <EmptyState icon="inbox" title="Sin transacciones en el período">
            No hay compras registradas para {MESES[Number(mes) - 1]} {anio}.
          </EmptyState>
        ) : (
          <div className="tbl-wrap">
            <table className="tbl">
              <thead>
                <tr>
                  <th>Suplidor</th><th>RNC</th><th>Tipo</th><th>NCF</th><th>Fecha</th>
                  <th className="num">Bienes</th><th className="num">Servicios</th>
                  <th className="num">ITBIS</th><th className="num">Total</th>
                </tr>
              </thead>
              <tbody>
                {data?.registros.map((r, i) => (
                  <tr key={`${r.ncf}-${i}`}>
                    ...
                    <td className="num text-sm"><Money value={r.monto_bienes} cur={false} /></td>
                    <td className="num text-sm"><Money value={r.monto_servicios} cur={false} /></td>
                    <td className="num text-sm muted"><Money value={r.itbis_facturado} cur={false} /></td>
                    <td className="num fw6"><Money value={r.total_facturado} cur={false} /></td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr style={{ background: 'var(--surface-2)' }}>
                  <td colSpan={5} className="fw6 text-sm" style={{ padding: '11px 14px' }}>
                    Totales ({data?.cantidad})
                  </td>
                  <td className="num fw6" style={{ padding: '11px 14px' }}><Money value={data?.totales.monto_bienes ?? 0} cur={false} /></td>
                  <td className="num fw6" style={{ padding: '11px 14px' }}><Money value={data?.totales.monto_servicios ?? 0} cur={false} /></td>
                  <td className="num fw6" style={{ padding: '11px 14px' }}><Money value={data?.totales.itbis_facturado ?? 0} cur={false} /></td>
                  <td className="num fw6" style={{ padding: '11px 14px' }}><Money value={data?.totales.total_facturado ?? 0} cur={false} /></td>
                </tr>
              </tfoot>
            </table>
          </div>
